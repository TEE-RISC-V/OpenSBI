...
/**
 * Redirect interrupt trap to lower privilege mode (S-mode or U-mode)
 *
 ...
 * @return 0 on success and negative error code on failure
 */
int sbi_interrupt_redirect(struct sbi_trap_regs *regs,
		      struct sbi_trap_info *trap)
{
	...
	if (next_virt) {
		...
		// sbi_printf("redirecting to supervisor\n");
		...
		if (prev_virt && !next_virt && !scratch->storing_vcpu) {
			// sbi_printf("not storing vcpu...\n");
		}
...
struct sbi_trap_regs *sbi_trap_handler(struct sbi_trap_regs *regs)
{
	...
	if (mcause & (1UL << (__riscv_xlen - 1))) {
		...
		if (rc) {
			...
			rc = sbi_interrupt_redirect(regs, &trap);
		}
